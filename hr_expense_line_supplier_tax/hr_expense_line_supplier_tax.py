...
class hr_expense_line(orm.Model):
    ...
    def onchange_product_id(self, cr, uid, ids, product_id, uom_id,
        ...
        if product_id:
            ...
            values = parent_h_e_l.onchange_product_id(cr,
                                                      uid,
                                                      ids,
                                                      product_id,
                                                      uom_id,
        ...
        for i_id in ids:
            this = self.browse(cr, uid, i_id)
            if this.name:
                tax_id = self.get_tax_id(cr, uid, product_id,
                                         this.partner_id.id, context)
                values['value'].update({
                    'tax_id': tax_id
                })
        return values
    ...
    def onchange_partner_id(self, cr, uid, ids, partner_id, context=None):
        ...
        for i_id in ids:
            this = self.browse(cr, uid, i_id)
            tax_id = self.get_tax_id(cr, uid, this.product_id.id, partner_id,
                                     context)
            values['value'].update({
                'tax_id': tax_id
            })
